...
namespace IMU{
    ...
    namespace {
        ...
        SPISettings _settingsMPU;
        SPISettings _settingsBMP;
        uint _selMPU;
        uint _selBMP;
        ...
        void Transfer( uint8_t Addr, uint8_t *buffer, unsigned int Bytes , bool toMPU)
        {
            unsigned int  i = 0;

            if(toMPU){
                digitalWrite(_selMPU,LOW);
                _spi->beginTransaction(_settingsMPU);
            }
            else{
                digitalWrite(_selBMP,LOW);
                _spi->beginTransaction(_settingsMPU);
            }
            ...
            _spi->transfer(buffer, Bytes);

            // _spi->write(NULL,0,buffer,Bytes);
            // for(i=0; i<Bytes; i++)
            ...
            if(toMPU)
                digitalWrite(_selMPU,HIGH);
            else
                digitalWrite(_selBMP,HIGH);
        }        

        void  WriteReg( uint8_t Addr, uint8_t Data , bool toMPU)
        {
            Transfer(Addr, &Data, 1, toMPU);
        }
    ...
    void init(SPIClass* spi, uint CSMPU, uint CSBMP){
        _spi=spi;
        _selMPU=CSMPU;
        _selBMP=CSBMP;

        pinMode(_selBMP, OUTPUT);
        pinMode(_selMPU, OUTPUT);
        digitalWrite(_selBMP, HIGH);
        digitalWrite(_selMPU, HIGH);
        _settingsBMP= SPISettings(1000000, SPI_MSBFIRST, SPI_MODE0);
        _settingsMPU= SPISettings(1000000, SPI_MSBFIRST, SPI_MODE0);
        ...
        for(int i=0; i<16; i++) {
            ...
            delay(1);  //let time for CS to go up and down between writes
        }
        ...
        delay(1); // wait for MPU to poll the slave
        uint8_t dataMagn[3];
        Transfer(MPUREG_EXT_SENS_DATA_00|READ_FLAG,dataMagn,3,true);
        // calibration factors
        for(int i = 0;i<3;i++){
    ...
    void getBMPData(float* table){
        Transfer(0x3B| READ_FLAG,dataIn,20,true);

        // reformat data 
        
        //accel in G (we could actually not do the scaling, the result is normed after)
        table[0]= ((float)((int16_t)(dataIn[0]<<8) + dataIn[1]))*1.220703125e-4f;
        table[1]= ((float)((int16_t)(dataIn[2]<<8) + dataIn[3]))*1.220703125e-4f;
        table[2]= ((float)((int16_t)(dataIn[4]<<8) + dataIn[5]))*1.220703125e-4f;

        //gyro in rad/s
        table[3]= ((float)((int16_t)(dataIn[8]<<8) + dataIn[9]))*2.6646248122052432e-4f;
        table[4]= ((float)((int16_t)(dataIn[10]<<8) + dataIn[11]))*2.6646248122052432e-4f;
        table[5]= ((float)((int16_t)(dataIn[12]<<8) + dataIn[13]))*2.6646248122052432e-4f;
    ...
    void calibrateSensor(uint led){
        ...
        while (nmes<200){
            ...
            Transfer(0x43| READ_FLAG,dataIn,6,true);

            data[0]= (int16_t)(dataIn[0]<<8) + dataIn[1];
            data[1]= (int16_t)(dataIn[2]<<8) + dataIn[3];
            data[2]= (int16_t)(dataIn[4]<<8) + dataIn[5];
        ...
        lastUpdate=micros();// initialize time interval with a non 0 value
